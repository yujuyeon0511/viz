#!/usr/bin/env python3
"""
Multi-Dataset InternVL Information Flow & Redundancy Visualization
Based on "From Redundancy to Relevance" (NAACL 2025)

Supports: ChartQA, DocVQA, KDTCBench, KOFFVQA

Usage:
    python visualize_multi_dataset.py \
...
def load_chartqa_samples(n_samples=20, split="human"):
    """Load from official ChartQA test set."""
    from config import CHARTQA_IMAGES, CHARTQA_HUMAN, CHARTQA_AUG
...
def load_docvqa_samples(n_samples=20):
    """Load from official DocVQA validation set (HuggingFace arrow format)."""
    from datasets import load_from_disk
    ds = load_from_disk("/NetDisk/juyeon/datasets/docvqa")
    val = ds["validation"]

    samples = []
    for i in range(min(n_samples, len(val))):
        item = val[i]
        # Save image to temp file for consistent interface
        img = item["image"]
        tmp_path = f"/tmp/docvqa_sample_{i}.png"
        img.save(tmp_path)
        ...
        samples.append({
            ...
            "id": f"docvqa_{item['questionId']}",
        })
    return samples


def load_kdtcbench_samples(n_samples=20):
    """Load from official K-DTCBench (Korean Document/Table/Chart Benchmark)."""
    import pandas as pd
    tsv_path = "/NetDisk/ingyu/VLMBenchmark/KDTCBench/data/K-DTCBench.tsv"
    ...
    for i, row in df.iterrows():
        ...
        img.load()
        tmp_path = f"/tmp/kdtcbench_sample_{i}.png"
        img.save(tmp_path)
        ...
        question = (
            ...
            f"주어진 선택지 중 해당 옵션의 문자로 바로 답하세요."
        )
        samples.append({
            "image_path": tmp_path,
            "question": question,
            "answer": str(row["answer"]),
            "id": f"kdtcbench_{row['index']}",
            "category": row.get("category", ""),
        })
    return samples


def load_koffvqa_samples(n_samples=20):
    """Load from official KOFFVQA (Korean Open-ended Free-form VQA)."""
    import pandas as pd
    tsv_path = "/NetDisk/ingyu/VLMBenchmark/KOFFVQA/data/KOFFVQA.tsv"
    df = pd.read_csv(tsv_path, sep="\t")
    ...
    for i, row in df.iterrows():
        ...
        img = Image.open(BytesIO(img_data))
        img.load()
        tmp_path = f"/tmp/koffvqa_sample_{i}.png"
        img.save(tmp_path)
        ...
        samples.append({
            ...
            "answer": str(row["answer"]),
            "id": f"koffvqa_{row['index']}",
            "category": row.get("category", ""),
        })
    return samples


def load_infovqa_samples(n_samples=20):
    """Load from official InfoVQA validation set (LMUData TSV format).
    Each unique image_path has image data only in its first occurrence.
    We cache decoded images and pick one question per unique image."""
    import pandas as pd
    tsv_path = "/NetDisk/juyeon/train/LMUData/InfoVQA_VAL.tsv"
    df = pd.read_csv(tsv_path, sep="\t")

    # Build image cache: image_path -> first row with valid base64
    image_cache = {}
    for i, row in df.iterrows():
        img_name = str(row["image_path"]).strip().strip('"')
        if img_name in image_cache:
            continue
        img_str = str(row["image"]).strip().strip('"')
        if len(img_str) >= 100:
            try:
                img_data = base64.b64decode(img_str)
                img = Image.open(BytesIO(img_data))
                img.load()
                tmp_path = f"/tmp/infovqa_{img_name}"
                img.save(tmp_path)
                image_cache[img_name] = tmp_path
            except Exception:
                continue

    # Pick one question per unique image
    samples = []
    seen_images = set()
    for i, row in df.iterrows():
        if len(samples) >= n_samples:
            break
        img_name = str(row["image_path"]).strip().strip('"')
        if img_name in seen_images or img_name not in image_cache:
            continue
        seen_images.add(img_name)

        answer = str(row["answer"]).strip().strip('"')
        question = str(row["question"]).strip().strip('"')
        samples.append({
            "image_path": image_cache[img_name],
            "question": question,
            "answer": answer,
            "id": f"infovqa_{row['index']}",
        })
    return samples


DATASET_LOADERS = {
    "chartqa": load_chartqa_samples,
    "docvqa": load_docvqa_samples,
    "infovqa": load_infovqa_samples,
    "kdtcbench": load_kdtcbench_samples,
    "koffvqa": load_koffvqa_samples,
}
...
def plot_vit_attention_heatmaps(attention_maps, original_image, output_path, model_name, num_layers_to_show=6):
    ...
    fig.suptitle(f"[{model_name}] InternViT Layer-wise Attention Heatmaps", fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
...
def plot_information_flow(hidden_states, img_positions, output_path, model_name):
    ...
    cosine_sims, l2_norms, variances, mean_token_sims = [], [], [], []
    ...
    ax.set_xlabel("Layer"); ax.set_ylabel("Mean Pairwise Cosine Sim")
...
def plot_token_redundancy_matrices(hidden_states, img_positions, output_path, model_name, layers_to_show=None):
    ...
    max_tokens = 64
    img_idx_sub = img_idx[::max(1, len(img_idx) // max_tokens)][:max_tokens] if len(img_idx) > max_tokens else img_idx
    ...
    fig.suptitle(f"[{model_name}] Image Token Redundancy (Cosine Similarity)", fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches='tight', facecolor='white')
    plt.close()
    print(f"  Saved: {output_path}")
...
def plot_comprehensive_dashboard(
    ...
    ax.imshow(img_np)
    ax.set_title("Image", fontsize=11, fontweight='bold')
    ax.axis('off')
    qa_text = f"Q: {question[:80]}{'...' if len(question) > 80 else ''}\nGT: {answer}\nPred: {model_answer}"
    ax.text(0.5, -0.15, qa_text, transform=ax.transAxes, fontsize=8,
            ha='center', va='top', bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.8), wrap=True)
    ...
    fig.suptitle(f"[{model_name}] {dataset_name} — Information Flow & Redundancy",
...
def plot_aggregate_analysis(all_results, output_path, model_name, dataset_name):
    ...
    if all_cosine:
        ...
        if cliff_layers:
            ...
            ax.set_title("Info Flow Cliff Layer Distribution", fontweight='bold')
            ax.legend(); ax.grid(True, alpha=0.3)

    fig.suptitle(f"[{model_name}] Aggregate Analysis on {dataset_name}", fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches='tight', facecolor='white')
    plt.close()
    print(f"  Saved: {output_path}")
...
def main():
    parser = argparse.ArgumentParser(description="Multi-Dataset InternVL Visualization")
    ...
    if args.output_dir is None:
        args.output_dir = f"outputs/{args.dataset}_{args.model_name}_{args.n_samples}samples"
    ...
    for idx, sample in enumerate(samples):
        print(f"\n{'='*60}")
        print(f"Sample {idx+1}/{len(samples)}: {sample['id']}")
        ...
        if not args.skip_llm:
            ...
            try:
                ...
                plot_comprehensive_dashboard(
                    ...
                    args.model_name, args.dataset.upper()
                )
    ...
    answers_path = os.path.join(output_dir, "answers_summary.json")
    summary = []
    for idx, sample in enumerate(samples):
        summary.append({
            "idx": idx,
    ...
    print(f"Answers summary saved to: {answers_path}")
