#!/usr/bin/env python3
"""
InternVL3.5-8B × ChartQA Visualization
Based on "From Redundancy to Relevance" (NAACL 2025)

Visualizations:
1. Layer-wise Attention Heatmap (GradCAM-style)
...
Usage:
    python visualize_internvl_chartqa.py --n_samples 5
    python visualize_internvl_chartqa.py --n_samples 10 --output_dir outputs/chartqa_viz
"""
...
from config import (
    INTERNVL_MODEL_PATH as MODEL_PATH,
...
def load_model():
    """Load InternVL3.5-8B with eager attention for extracting attention weights."""
    from transformers import AutoModel, AutoTokenizer

    print("Loading InternVL3.5-8B (eager attention for visualization)...")
    tokenizer = AutoTokenizer.from_pretrained(MODEL_PATH, trust_remote_code=True)
    model = AutoModel.from_pretrained(
        MODEL_PATH,
        torch_dtype=torch.bfloat16,
        trust_remote_code=True,
        use_flash_attn=False,  # Use eager attention to get attn weights
    ).eval().cuda()
...
def load_and_preprocess_image(image_path, image_size=448):
    """Load and preprocess an image for InternVL."""
...
# ═══════════════════════════════════════════════════════════════
# 1. Vision Encoder Attention Heatmap
# ═══════════════════════════════════════════════════════════════
...
def plot_vit_attention_heatmaps(attention_maps, original_image, output_path, num_layers_to_show=6):
    """
    Plot ViT attention heatmaps overlaid on the original image.
    Shows [CLS] token attention over patch tokens for selected layers.
    """
    ...
    fig.suptitle("InternViT Layer-wise Attention Heatmaps (CLS → Patches)", fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
...
def plot_information_flow(hidden_states, img_positions, output_path):
    ...
    Metrics (from "From Redundancy to Relevance"):
    - Cosine similarity between adjacent layers for image tokens
    - L2 norm of image token representations per layer
    - Variance of image tokens per layer (information convergence/divergence)
    """
    ...
    fig.suptitle("Information Flow Analysis (LLM Layers)", fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches='tight', facecolor='white')
    plt.close()
    print(f"  Saved: {output_path}")
...
def plot_token_redundancy_matrices(hidden_states, img_positions, output_path, layers_to_show=None):
    ...
    fig.suptitle("Image Token Redundancy (Cosine Similarity Matrices)", fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches='tight', facecolor='white')
    plt.close()
    print(f"  Saved: {output_path}")
...
def plot_comprehensive_dashboard(
    ...
    attention_maps, hidden_states, attentions, img_positions,
    output_path
):
    """Create a comprehensive dashboard combining all visualizations for one sample."""
    ...
    fig.suptitle("InternVL3.5-8B × ChartQA — Information Flow & Redundancy Analysis",
...
def plot_aggregate_analysis(all_results, output_path):
    """Plot aggregate analysis across all samples."""
    ...
    fig.suptitle("Aggregate Analysis: InternVL3.5-8B on ChartQA", fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches='tight', facecolor='white')
    plt.close()
    print(f"  Saved: {output_path}")
...
def main():
    parser = argparse.ArgumentParser(description="InternVL3.5-8B ChartQA Visualization")
    parser.add_argument("--n_samples", type=int, default=5, help="Number of ChartQA samples to visualize")
    parser.add_argument("--split", type=str, default="human", choices=["human", "augmented"])
    parser.add_argument("--output_dir", type=str, default="outputs/chartqa_internvl_viz")
        ...
        if not args.skip_llm:
            ...
            try:
                ...
                plot_comprehensive_dashboard(
                    ...
                    attention_maps, hidden_states, None, img_positions,
